...
5. **הגדר את הפרויקט:**
   ...
   - **Root Directory:** `frontend` ⚠️ **חשוב מאוד!**
     - לחץ על "Configure Project"
     - ב-"Root Directory" → הזן: `frontend`
     - זה חיוני כי ה-package.json נמצא ב-`frontend/`
   - **Build Command:** `npm run build` (אוטומטי אחרי Root Directory)
